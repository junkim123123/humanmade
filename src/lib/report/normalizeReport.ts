...
export function getSupplierMatches(report: any): NormalizedSupplierMatch[] {
  ...
  if (Array.isArray(report._supplierMatches) && report._supplierMatches.length > 0) {
    return report._supplierMatches.map(normalizeMatch);
  }
  ...
  if (recommended.length > 0 || candidates.length > 0) {
    return [...recommended, ...candidates].map(normalizeMatch);
  }
  ...
  if (Array.isArray(legacyMatches) && legacyMatches.length > 0) {
    return legacyMatches.map(normalizeMatch);
  }
 ...
 */
function normalizeMatch(match: any): NormalizedSupplierMatch {
  // Extract supplier_id and supplier_name (handle various field names)
  const supplier_id = match.supplier_id || match.supplierId || match.id || "";
  const supplier_name = match.supplier_name || match.supplierName || match.companyName || "Unknown";
